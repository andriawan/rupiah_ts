class Rupiah {
    ...
    get format() : string {
        ...
        [...this.rawNumber.toString()].reverse().forEach((value, index, array) => {
            rupiah += value;
            if((index + 1) %3 == 0 && (index + 1 != array.length)) rupiah += this.separator;
          });
    ...
    get terbilang() : string {
        return this.chunk(this.rawNumber).map(this.indonesianTerbilang).map(this.appendBilanganBesar).filter(this.isTruthy).reverse().join(" ") + " Rupiah";
    }
